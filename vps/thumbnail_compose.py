...
def compose_thumbnail_text(
    ...
    *,
    ...
    for word in words:
        ...
        if (bbox[2] - bbox[0]) > w * 0.42 and current:
    ...
    for i, line in enumerate(lines):
        lw = line_widths[i]
        x = (w - margin_x - lw) if position == "right" else margin_x
        ...
        if i == 0:
            ...
            scrim_draw.rectangle(
                [max(0, x - int(w * 0.03)), y_start - int(h * 0.04),
                 min(w, x + int(w * 0.44)), y_start + block_h + int(h * 0.04)],
                fill=(0, 0, 0, 110),
            )
        ...
        if i == len(lines) - 1 and " " in line:
            white_part, accent = line.rsplit(" ", 1)
            white_part += " "
            draw.text((x, y), white_part, font=font, fill=(255, 255, 255, 255),
                      stroke_width=stroke, stroke_fill=(0, 0, 0, 255))
            bbox_wp = draw.textbbox((x, y), white_part, font=font)
            draw.text((bbox_wp[2], y), accent, font=font, fill=(220, 38, 38, 255),
                      stroke_width=stroke, stroke_fill=(0, 0, 0, 255))
        else:
