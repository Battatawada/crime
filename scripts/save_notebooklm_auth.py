...
Usage:
  python scripts/save_notebooklm_auth.py --profile crime
  notebooklm -p crime auth check --test
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--profile",
        default="crime",
        help="notebooklm-py profile name (default: crime — do not reuse Niche default)",
    )
    ...
    print("Open browser and sign in with the CRIME Google account (not Niche/Psychology).")
    ...
    with sync_playwright() as p:
        ...
        input("\nWhen NotebookLM homepage is visible (crime account), press ENTER to save auth... ")
